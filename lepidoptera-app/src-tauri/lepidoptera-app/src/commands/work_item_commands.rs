...
pub fn get_work_item(
    state: State<'_, Arc<AppContext>>,
    id: String,
) -> Result<Option<WorkItemModel>, String> {
    let command_name = "get_work_item";
    debug!("[COMMAND] {} called: id={}", command_name, id);
    let start = std::time::Instant::now();
    
    let ctx = state.inner();
    let work_items_manager = ctx.work_items.clone();
    
    match work_items_manager.get_work_item(&id) {
        Ok(result) => {
            let duration = start.elapsed();
            match &result {
                Some(_) => {
                    info!("[COMMAND] {} completed successfully in {:?} (found work item)", command_name, duration);
                }
                None => {
                    info!("[COMMAND] {} completed successfully in {:?} (work item not found)", command_name, duration);
                }
            }
            Ok(result)
        }
